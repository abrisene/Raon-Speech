# Seanet encoder/decoder for audio codec.
    ...
    def __init__(self, cfg: SeanetConfig, dim: int, ksizes_and_dilations: list):
        ...
        for i, (ksize, dilation) in enumerate(ksizes_and_dilations):
            ...
            c = StreamableConv1d(
                ...
                ksize=ksize, stride=1, dilation=dilation, groups=1,
                bias=True, causal=cfg.causal, pad_mode=cfg.pad_mode,
            )
            block.append(c)
        self.block = block
        ...
        if cfg.true_skip:
            ...
            self.shortcut = StreamableConv1d(
                in_channels=dim, out_channels=dim, ksize=1, stride=1,
                dilation=1, groups=1, bias=True, causal=cfg.causal, pad_mode=cfg.pad_mode,
            )
...
class EncoderLayer(nn.Module):
    def __init__(self, cfg: SeanetConfig, ratio: int, mult: int):
        ...
        self.downsample = StreamableConv1d(
            in_channels=mult * cfg.nfilters, out_channels=mult * cfg.nfilters * 2,
            ksize=ratio * 2, stride=ratio, dilation=1, groups=1,
            bias=True, causal=True, pad_mode=cfg.pad_mode,
        )
...
class SeanetEncoder(nn.Module):
    def __init__(self, cfg: SeanetConfig):
        ...
        self.init_conv1d = StreamableConv1d(
            in_channels=cfg.channels, out_channels=mult * cfg.nfilters,
            ksize=cfg.ksize, stride=1, dilation=1, groups=1,
            bias=True, causal=cfg.causal, pad_mode=cfg.pad_mode,
        )
        layers = []
        ...
        self.final_conv1d = StreamableConv1d(
            in_channels=mult * cfg.nfilters, out_channels=cfg.dimension,
            ksize=cfg.last_ksize, stride=1, dilation=1, groups=1,
            bias=True, causal=cfg.causal, pad_mode=cfg.pad_mode,
        )
...
class DecoderLayer(nn.Module):
    def __init__(self, cfg: SeanetConfig, ratio: int, mult: int):
        ...
        self.upsample = StreamableConvTranspose1d(
            in_channels=mult * cfg.nfilters, out_channels=mult * cfg.nfilters // 2,
            ksize=ratio * 2, stride=ratio, groups=1, bias=True, causal=cfg.causal,
        )
        residuals = []
        dilation = 1
        for _ in range(cfg.nresidual_layers):
...
class SeanetDecoder(nn.Module):
    def __init__(self, cfg: SeanetConfig):
        ...
        self.init_conv1d = StreamableConv1d(
            in_channels=cfg.dimension, out_channels=mult * cfg.nfilters,
            ksize=cfg.ksize, stride=1, dilation=1, groups=1,
            bias=True, causal=cfg.causal, pad_mode=cfg.pad_mode,
        )
        layers = []
        ...
        self.layers = layers
        self.final_conv1d = StreamableConv1d(
            in_channels=cfg.nfilters, out_channels=cfg.channels,
            ksize=cfg.last_ksize, stride=1, dilation=1, groups=1,
            bias=True, causal=cfg.causal, pad_mode=cfg.pad_mode,
        )
