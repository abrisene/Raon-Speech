# Conv1d and streaming conv modules for MLX.
...
def get_extra_padding_for_conv1d(
    xs: mx.array, ksize: int, stride: int, padding_total: int,
) -> int:
    len_ = xs.shape[-1]
...
class StreamableConv1d(nn.Module):
    def __init__(
        ...
        self.conv = NormConv1d(
            in_channels, out_channels, ksize,
            stride=stride, groups=groups, dilation=dilation, bias=bias,
        )
        self._prev_xs = None
    ...
    def __call__(self, xs: mx.array) -> mx.array:
        ...
        extra_padding = get_extra_padding_for_conv1d(
            xs, ksize=ksize, stride=self.conv.conv._stride, padding_total=padding_total,
        )
        z = 0, 0
        if self._causal:
...
class ConvDownsample1d(nn.Module):
    def __init__(self, stride: int, dim: int, causal: bool):
        ...
        self.conv = StreamableConv1d(
            in_channels=dim, out_channels=dim, ksize=2 * stride,
            stride=stride, dilation=1, groups=1, bias=False,
            causal=causal, pad_mode="edge",
        )
