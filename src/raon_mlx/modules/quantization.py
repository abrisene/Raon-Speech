# Vector quantization modules for audio codec.
...
class ResidualVectorQuantization(nn.Module):
    def __init__(self, nq: int, dim: int, codebook_size: int, codebook_dim: int | None):
        ...
        layers = []
        for _ in range(nq):
            vq = VectorQuantization(dim=dim, codebook_size=codebook_size, codebook_dim=codebook_dim)
            layers.append(vq)
        self.layers = layers
...
class ResidualVectorQuantizer(nn.Module):
    def __init__(
        ...
        self.vq = ResidualVectorQuantization(nq=nq, dim=dim, codebook_size=bins, codebook_dim=None)
...
class SplitResidualVectorQuantizer(nn.Module):
    def __init__(
        ...
        self.rvq_first = ResidualVectorQuantizer(
            dim=dim, input_dim=input_dim, output_dim=output_dim,
            nq=1, bins=bins, force_projection=True,
        )
        self.rvq_rest = ResidualVectorQuantizer(
            dim=dim, input_dim=input_dim, output_dim=output_dim,
            nq=nq - 1, bins=bins, force_projection=True,
        )
